...
public class Main extends Activity {
    ...
    private JoystickMovedListener _listener = new JoystickMovedListener() {
        ...
        public void OnMoved(int pan, int tilt) {
            if (connected) {
                try {
                    dataOutputStream.writeBytes((pan + 90) + " A");
                } catch (UnknownHostException e) {
                } catch (IOException e) {
                }
            }
            if (connected) {
                try {
                    dataOutputStream.writeBytes((tilt + 90) + " B");
                } catch (UnknownHostException e) {
                } catch (IOException e) {
                }
            }
        ...
        public void OnReleased() {
            if (connected) {
                try {
                    dataOutputStream.writeBytes(90 + " A");
                } catch (UnknownHostException e) {
                } catch (IOException e) {
                }
            }
            if (connected) {
                try {
                    dataOutputStream.writeBytes(90 + " B");
                } catch (UnknownHostException e) {
                } catch (IOException e) {
                }
            }
    ...
    Button.OnClickListener buttonConnectOnClickListener = new Button.OnClickListener() {
        ...
        public void onClick(View arg0) {
            ...
            if (wifiInfo.isConnected()) {
                if (!connected) {
                    try {
                        socket = new Socket("192.168.1.120", 55555);
    ...
    public void changeConnectionStatus(Boolean isConnected) {
        ...
        if (isConnected) {
            ...
            buttonConnect.setBackgroundResource(R.drawable.con);
